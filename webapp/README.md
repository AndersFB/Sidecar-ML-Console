...
- The Faces and Pose panels have a **Live camera** mode: webcam frames stream
  to the phone one at a time (JPEG, one request in flight) and detections are
  drawn over the live preview. Like the microphone, the camera needs a secure
  context — `http://localhost` or the downloaded single file.
